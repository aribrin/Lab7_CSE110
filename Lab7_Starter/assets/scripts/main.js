...
async function getRecipes() {
  ...
  var recipeList = (localStorage.getItem('recipes'));
  if(recipeList) {
  
  var rx =  /[\[\]']+/g;
  var rx2 = /\}\,\{+/g;
  var prunedList = (recipeList.replace(rx,"")).replace(rx2,"}},{{").split(/\}\,\{/);

    //forEach((element) => { /* … */ })
    var recipeObjList = [];
    prunedList.forEach((recipe) => {recipeObjList.push(JSON.parse(recipe));});
  ...
  // we're returning
  /**************************/
  const prommy = new Promise(async () => 
  {
    ...
    for( var i = 0; i < RECIPE_URLS.length; i++){
      try{
        ...
        //            funct ion is using the async keyword so we recommend "await"
